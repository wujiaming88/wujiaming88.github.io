...
| `dreaming.frequency` | `"0 3 * * *"` | sweep 频率（可改为 `"0 */6 * * *"` 或 `"0 3 * * 1"`） |
...
Deep Phase 评分 **69% 权重**依赖 memory_search 的召回追踪。禁用 memorySearch 会导致 Dreaming 几乎无法晋升。最省钱方案：不配 embedding provider → FTS5 自动可用 → 零 API 成本。
