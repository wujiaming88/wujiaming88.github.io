...
如果把本周的 Agent 基础设施动态串成一条线，会看到三股力量在同一个方向上较劲：**三大云厂**比拼托管平台的组件密度与商业落地，**模型厂商与通用框架**把竞争从开源 SDK 上移到托管平台与治理平面，**数据/开源/中国平台**则争抢多 Agent 的"控制平面"。下面依次展开这三条战线，最后收束本周的格局判断。
